    ...
    function showUnlockModal({ mode, alg, purpose, errorMessage = "" }) {
        ...
        return new Promise((resolve) => {
            ...
            const purposeText = purpose === "enroll"
                ? tr("pqshare.unlock.purpose_enroll", null, "This browser is about to create a local device key for post-quantum share enrollment.")
                : tr("pqshare.unlock.purpose_open", null, "This browser needs to unlock its local device key before it can open this protected share.");
            ...
            backdrop.innerHTML = `
              <div class="pqsu-modal" role="dialog" aria-modal="true" aria-labelledby="pqsu-title">
                ...
                <div class="pqsu-body">
                  ...
                  <div class="pqsu-warn">
                    ${escapeHtml(tr("pqshare.unlock.warning", { alg }, `This passphrase protects your local ${alg} device key stored in this browser. It is never sent to the server and cannot be recovered. If you forget it, this browser will no longer be able to open shares encrypted to this device key. You can reset and re-enroll this browser for future shares, but older shares tied to the old key will remain inaccessible here.`))}
                  </div>

                  <div class="pqsu-field">
                    <label class="pqsu-label" for="pqsu-secret">${isSetup ? escapeHtml(tr("pqshare.unlock.create_passphrase", null, "Create passphrase")) : escapeHtml(tr("pqshare.unlock.enter_passphrase", null, "Enter passphrase"))}</label>
                    <input id="pqsu-secret" class="pqsu-input" type="password" autocomplete="new-password">
                  </div>

                  ${isSetup ? `
                    <div class="pqsu-field">
                      <label class="pqsu-label" for="pqsu-secret2">${escapeHtml(tr("pqshare.unlock.confirm_passphrase", null, "Confirm passphrase"))}</label>
                    ...
                    <label class="pqsu-check">
                      ...
                      <span>${escapeHtml(tr("pqshare.unlock.ack", null, "I understand this passphrase cannot be recovered."))}</span>
                    </label>
                  ` : ""}
                  ...
                  <div class="pqsu-actions">
                    <button id="pqsu-submit" class="pqsu-btn pqsu-btn-primary" type="button">
                      ${isSetup ? escapeHtml(tr("pqshare.unlock.create_continue", null, "Create and continue")) : escapeHtml(tr("pqshare.unlock.unlock", null, "Unlock"))}
                    </button>
                    <button id="pqsu-cancel" class="pqsu-btn pqsu-btn-secondary" type="button">${escapeHtml(tr("admin.common.cancel", null, "Cancel"))}</button>
                    ${!isSetup ? `<button id="pqsu-reset" class="pqsu-btn pqsu-btn-danger" type="button">${escapeHtml(tr("pqshare.unlock.reset_key", null, "Reset this browser key"))}</button>` : ""}
                  </div>
                </div>
              </div>
            `;
            ...
            function submit() {
                ...
                if (!secret) {
                    errorEl.textContent = tr("pqshare.unlock.passphrase_required", null, "Passphrase is required.");
                    return;
                }
                ...
                if (isSetup) {
                    ...
                    if (secret.length < 8) {
                        errorEl.textContent = tr("pqshare.unlock.min_chars", null, "Use at least 8 characters.");
                        return;
                    }
                    if (secret !== secret2) {
                        errorEl.textContent = tr("pqshare.unlock.no_match", null, "Passphrases do not match.");
                        return;
                    }
                    if (!ackEl?.checked) {
                        errorEl.textContent = tr("pqshare.unlock.confirm_warning", null, "Please confirm that you understand the recovery warning.");
                        return;
                    }
                }
    ...
    async function ensureUnlocked({ preferredAlg = "ML-KEM-768", purpose = "open" } = {}) {
        ...
        for (;;) {
            ...
            if (!result || result.action === "cancel") {
                throw new Error(tr("pqshare.unlock.required", null, "Device key passphrase is required"));
            }
            ...
            try {
                ...
                errorMessage = String(err && err.message ? err.message : err) || tr("pqshare.unlock.failed", null, "Unlock failed");
            }
        }
    }
