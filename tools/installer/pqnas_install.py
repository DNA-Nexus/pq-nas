...
def install_pqnas_btrfs_snapshot_sudoers(log: Optional[Log] = None) -> None:
    """
    Allow pqnas_server to run only the guarded Btrfs snapshot helper as root.

    The helper validates source and destination paths and permits only PQ-NAS
    managed snapshot roots, rather than exposing direct sudo access to btrfs.
    """
    content = "pqnas ALL=(root) NOPASSWD: /usr/local/sbin/pqnas-btrfs-snapshot *"
    install_sudoers_rule("pqnas-btrfs-snapshot", content, log=log)


def install_pqnas_btrfs_status_sudoers(log: Optional[Log] = None) -> None:
    """
    Allow pqnas_server to run only the guarded Btrfs read-only status helper as root.

    The helper validates mount paths and exposes only non-mutating btrfs status
    commands for PQ-NAS managed mount roots.
    """
    content = "pqnas ALL=(root) NOPASSWD: /usr/local/sbin/pqnas-btrfs-status *"
...
def install_btrfs_snapshot_assets(asset_root: str, log: Optional[Log] = None) -> str:
    """
    Install guarded Btrfs snapshot helper.

    Package layout supported:
      <asset_root>/sbin/pqnas-btrfs-snapshot
      <asset_root>/libexec/pqnas/pqnas-btrfs-snapshot

    Repo layout supported:
      <asset_root>/server/src/storage/pqnas_btrfs_snapshot_root.sh
    """
    helper_src = _first_existing_path([
        os.path.join(asset_root, "sbin", "pqnas-btrfs-snapshot"),
        os.path.join(asset_root, "libexec", "pqnas", "pqnas-btrfs-snapshot"),
        os.path.join(asset_root, "server", "src", "storage", "pqnas_btrfs_snapshot_root.sh"),
    ])

    if not helper_src:
        raise RuntimeError(
            "Btrfs snapshot helper not found in package/repo assets. "
            "Expected pqnas-btrfs-snapshot under sbin/libexec or "
            "server/src/storage/pqnas_btrfs_snapshot_root.sh."
        )

    helper_dst = "/usr/local/sbin/pqnas-btrfs-snapshot"
    ...
    install_pqnas_btrfs_snapshot_sudoers(log=log)

    if log:
        log.write(f"[*] Installed Btrfs snapshot helper: {helper_dst}")

    return helper_dst


def install_btrfs_status_assets(asset_root: str, log: Optional[Log] = None) -> str:
    """
    Install guarded Btrfs read-only status helper.

    Package layout supported:
      <asset_root>/sbin/pqnas-btrfs-status
      <asset_root>/libexec/pqnas/pqnas-btrfs-status

    Repo layout supported:
      <asset_root>/server/src/storage/pqnas_btrfs_status_root.sh
    """
    helper_src = _first_existing_path([
        os.path.join(asset_root, "sbin", "pqnas-btrfs-status"),
        os.path.join(asset_root, "libexec", "pqnas", "pqnas-btrfs-status"),
        os.path.join(asset_root, "server", "src", "storage", "pqnas_btrfs_status_root.sh"),
    ])

    if not helper_src:
        raise RuntimeError(
            "Btrfs status helper not found in package/repo assets. "
            "Expected pqnas-btrfs-status under sbin/libexec or "
            "server/src/storage/pqnas_btrfs_status_root.sh."
        )

    helper_dst = "/usr/local/sbin/pqnas-btrfs-status"
    os.makedirs(os.path.dirname(helper_dst), exist_ok=True)
    ...
    install_pqnas_btrfs_status_sudoers(log=log)
...
def write_env_file(
    ...
    lines = [
        ...
        "",
        "# Optional SMTP settings for Notifications + Warnings email delivery.",
        "# If PQNAS_SMTP_HOST is empty, pqnas_notify.py falls back to local sendmail/msmtp.",
        ...
        "",
        "# Circle Stack / Nodus federation worker.",
        "# Enabled by default so public Circle Stack federation is active after install.",
        "PQNAS_CIRCLE_FEDERATION_WORKER=1",
        "PQNAS_CIRCLE_FEDERATION_WORKER_SEED=all",
        "PQNAS_CIRCLE_FEDERATION_WORKER_CIRCLE_ID=local-public-feed",
    ]
