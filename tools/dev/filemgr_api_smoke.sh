...
run_user_suite() {
  ...
  if [[ "$vc" -gt 0 ]]; then
    ...
    expect_raw_status "versions/download first" "GET" "/api/v4/files/versions/download?path=$(urlenc "$text1")&version_id=$(urlenc "$vid")" "200" -o "$dl_version"
  ...
  expect_raw_status "zip test root" "GET" "/api/v4/files/zip?path=$(urlenc "$dir")&max_bytes=10485760" "200" -o "$TMPDIR/root.zip"
...
run_workspace_suite() {
  ...
  expect_json_ok_or_not_found "workspace delete old root" "POST" "/api/v4/workspaces/files/delete?workspace_id=$qws&path=$(urlenc "$wroot")"
  ...
  expect_raw_status "workspace get file" "GET" "/api/v4/workspaces/files/get?workspace_id=$qws&path=$(urlenc "$wfile")" "200" -o "$TMPDIR/ws-download.txt"
  ...
  if [[ "$KEEP" == "1" ]]; then
    ...
    expect_json_ok_or_not_found "workspace delete root cleanup" "POST" "/api/v4/workspaces/files/delete?workspace_id=$qws&path=$(urlenc "$wroot")"
  fi
}
