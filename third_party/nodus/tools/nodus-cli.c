 ...
 *   identity-init              Generate and save identity into -i <identity_dir>
 */
...
static void chmod_identity_file(const char *dir, const char *name, mode_t mode) {
    char path[512];
    snprintf(path, sizeof(path), "%s/%s", dir, name);
    chmod(path, mode);
}

static int cmd_identity_init(const char *identity_dir) {
    if (!identity_dir || !identity_dir[0]) {
        fprintf(stderr, "identity-init requires -i <identity_dir>\n");
        return 2;
    }

    if (mkdir(identity_dir, 0700) != 0 && errno != EEXIST) {
        fprintf(stderr, "Failed to create identity dir %s: %s\n",
                identity_dir, strerror(errno));
        return 1;
    }

    nodus_identity_t existing;
    if (nodus_identity_load(identity_dir, &existing) == 0) {
        printf("Identity already exists: %s\n", existing.fingerprint);
        nodus_identity_clear(&existing);
        return 0;
    }
    ...
    if (nodus_identity_save(&id, identity_dir) != 0) {
        fprintf(stderr, "Failed to save identity to %s\n", identity_dir);
        nodus_identity_clear(&id);
        return 1;
    }

    chmod(identity_dir, 0700);
    chmod_identity_file(identity_dir, "nodus.pk", 0600);
    chmod_identity_file(identity_dir, "nodus.sk", 0600);
    chmod_identity_file(identity_dir, "nodus.fp", 0600);
    chmod_identity_file(identity_dir, "nodus.kyber_pk", 0600);
    chmod_identity_file(identity_dir, "nodus.kyber_sk", 0600);
...
static void usage(const char *prog) {
    ...
    fprintf(stderr, "  identity-init    Generate and save identity into -i <identity_dir> if missing\n");
...
int main(int argc, char **argv) {
    ...
    /* Handle identity-init without server */
    if (strcmp(command, "identity-init") == 0) {
        int rc = cmd_identity_init(identity_dir);
        return rc;
    }
