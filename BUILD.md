...
# PQNAS_SERVER_SK_B64URL is generated by pqnas_keygen and written to /etc/pqnas/keys.env.
# PQNAS_COOKIE_KEY_B64URL is generated by pqnas_keygen and written to /etc/pqnas/keys.env.
```
...
The DNA library used by server-side identity generation is loaded from the fixed
root-managed path `/opt/pqnas/lib/dna/libdna_lib.so`. Do not set a
`PQNAS_DNA_LIB` environment override for the server process.
